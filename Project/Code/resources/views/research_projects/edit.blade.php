...
<div class="container">
    ...
    <div class="card col-md-12" style="padding: 16px;">
        ...
        <p class="card-description">{{ __('message.fill_project_details') }}</p>
            <form action="{{ route('researchProjects.update',$researchProject->id) }}" method="POST">
                ...
                <div class="form-group row">
                <p class="col-sm-3 "><b>{{ __('message.project_start_date') }}</b></p>
                <div class="col-sm-4">
                        <input type="date" name="project_start" value="{{ $researchProject->project_start }}" class="form-control">
                    </div>
                </div>
                <div class="form-group row">
                <p class="col-sm-3 "><b>{{ __('message.project_end_date') }}</b></p>
                <div class="col-sm-4">
                        <input type="date" name="project_end" value="{{ $researchProject->project_end }}" class="form-control">
                    </div>
                </div>
                <div class="form-group row mt-2">
                <p for="exampleInputfund_details" class="col-sm-3"><b>{{ __('message.select_funding') }}</b></p>
                <div class="col-sm-9">
                        <select id='fund' style='width: 200px;' class="custom-select my-select" name="fund">
                            <option value='' disabled selected>{{ __('message.select_funding') }}</option>@foreach($funds as $f)<option value="{{ $f->id }}" {{ $f->fund_name == $researchProject->fund->fund_name ? 'selected' : '' }}>{{ $f->fund_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-sm-8">
                        <input type="year" name="project_year" class="form-control" placeholder="{{ __('message.year') }}" value="{{$researchProject->project_year}}">
                </div>
                </div>
                <div class="form-group row">
                ...
                <div class="col-sm-3">
                        <select id='dep' style='width: 200px;' class="custom-select my-select"  name="responsible_department">
                            <option value=''>{{ __('message.select_department') }}</option>@foreach($deps as $dep)<option value="{{ $dep->department_name_th }}" {{ $dep->department_name_th == $researchProject->responsible_department ? 'selected' : '' }}>{{ $dep->department_name_th }}</option>@endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-sm-8">
                        ...
                        <option value="3" {{ 3 == $researchProject->status ? 'selected' : '' }}>{{ __('message.status_completed') }}</option>
                        </select>
                    </div>
                </div>
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <table class="table">
                        <tr>
                            <th>{{ __('message.project_leader') }}</th>
                        <tr>
                            <td>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 mb-3">
                    <table class="table " id="dynamicAddRemove">
                        <tr>
                            <th width="522.98px">{{ __('message.project_assistant_internal') }}</th>
                            <th><button type="button" name="add" id="add-btn2" class="btn btn-success btn-sm add"><i class="mdi mdi-plus"></i></button></th>
                        </tr>
                    </table>
                </div>
                <div class="form-group row">
                        <label for="exampleInputpaper_author" class="col-sm-3 col-form-label">{{ __('message.project_assistant_external') }}</label>
                        <div class="col-sm-9">
                ...
                <a class="btn btn-light mt-5" href="{{ route('researchProjects.index') }}"> {{ __('message.back') }}</a>
            </form>
        </div>
    </div>
</div>
